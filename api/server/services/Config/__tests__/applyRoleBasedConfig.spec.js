...
jest.mock('~/cache/getLogStores', () => jest.fn().mockReturnValue(mockCache));
...
describe('applyRoleBasedConfig', () => {
  beforeEach(() => {
    ...
    mockCache.delete.mockReset().mockResolvedValue(true);
  });
  ...
  it('should return cached resolved role config before applying restrictions', async () => {
    ...
    mockCache.get.mockImplementation((key) => {
      ...
      if (key === '_OVERRIDE_:__default__:USER') {
